...
使用 `debug-<12 位提交哈希>` 标签。调试版的版本号基于该提交可访问的
最新 `X.Y.Z` 正式标签，例如 `4.5.0-debug.9`，APK 名称与该版本号一致。
新调试版发布成功后，工作流才会删除更早的 `debug-*` 以及迁移前遗留的
`preview-*` prerelease 和标签，仅保留最新版本。

推送严格匹配 `X.Y.Z` 的标签会在同一次工作流中从该标签构建、签名并发布
正式 GitHub Release，不依赖之前的 `main` 构建产物。仅修改 Markdown 或
`.trellis/**` 的 `main` 推送不会触发发布。

每个正式版本的发布说明存放在 `release-notes/<X.Y.Z>.md`。文件必须依次且
仅包含一个 `## 新增`、`## 删除` 和 `## 修复` 二级标题，每个章节至少包含
一条非空 Markdown 列表项；没有对应变更时写 `- 无`。正式标签工作流会先验证
与标签同名的文件，再将其作为完整 Release 正文发布。调试 prerelease 仍使用
自动生成的发布说明。

调试 prerelease 与正式版使用相同的 applicationId 和签名，因此可以直接升级
正式版并保留登录状态、设置和应用数据。调试 prerelease 启用调试并关闭混淆，
其缺陷可能影响现有数据。本地单元测试、静态检查和 lint 是代码质量门禁，APK
打包与签名验证由 GitHub Actions 完成，安装及功能验收由维护者手动完成。
