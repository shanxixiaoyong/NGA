...
- foundation 必须先通过 Justwen 原始 UI/导航/主题 smoke test、账号/会话契约、`minSdk 30` 构建和 Android 15/API 35 主设备验证。根目录迁移未完成、原始基线无法构建或会话契约未通过时，本任务保持 blocked，不得以模拟成功继续。
...
10. 保持 Justwen `minSdk = 30`、`compileSdk = 35`、`targetSdk = 35`；Android 15/API 35 是性能、并发、Compose/View 互操作、上传生命周期和完整读写/草稿流程的主验证门。API 30 最低安装/核心 smoke 与 API 36 上 `targetSdk 35` 前向验证仅在用户提供匹配实体设备时补充；不启动模拟器，不要求用户当前必须有 API 30/36 设备，`targetSdk 36` 升级另立任务。
...
- [ ] Android 15/API 35 全流程通过 unit/integration/Compose/instrumentation 验证，长文编辑、上传和消息分页无 ANR/OOM；可用时附 API 30 最低 smoke 与 API 36 前向验证报告。
