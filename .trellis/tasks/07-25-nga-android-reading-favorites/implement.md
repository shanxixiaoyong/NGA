...
8. 保持 `minSdk 29`、`compile/target 35`，在 Android 15/API 35 实体设备运行启动/滚动/解析/拖动 benchmark 并优化 bounded caches/recomposition；API 29 最低 smoke 与 API 36 上 `targetSdk 35` 前向 smoke 仅在已有匹配实体设备时补充，不启动模拟器。
...
可选设备命令仅在用户提供对应实体设备时运行：`ANDROID_SERIAL=<api29-serial> ./gradlew connectedDebugAndroidTest` 和 `ANDROID_SERIAL=<api36-serial> ./gradlew connectedDebugAndroidTest`。缺少 API 29/36 设备不阻塞；不得启动模拟器，API 36 仍验证 `targetSdk 35`。
