...
## 七、真机验证结果（4.11.0 发布后补记）

规划时用户选择不接设备，以下几点当时**未经真机确认**。4.11.0 发布后由用户在小米 15
（HyperOS）实测：

- ~~HyperOS / ColorOS / OriginOS 是否存在不经过 `Menu` 的额外浮层。~~
  **已确认**：小米 15 上分享按钮消失，厂商注入项走的是同一个 `Menu`，被
  `menu.clear()` 一并清掉。小米在几家里改系统改得最深，它上面干净说明这个接管点是
  可靠的；加上 WebView 本身是 Mainline 模块、厂商不改，vivo / OPPO / 三星不需要单独
  适配。
- `window.getSelection().selectAllChildren(document.body)` 在当前 HTML 模板结构下的
  全选范围是否符合预期 —— 仍未逐项验证。
- 复制富文本选区时只取纯文本是否影响实际使用体验 —— 仍未逐项验证。
